...
void main(void) {
    ...
    centeredCoord.x *= sin(streTexCoordhFactor * time);
    vec2 streTexCoordhedCoord = (centeredCoord + vec2(1.0, 1.0)) / 2.0;
    vec4 FragColor2 = texture(textTexture, streTexCoordhedCoord);

    float gliTexCoordhFactor = sin(time * 50.0);
    vec4 gliTexCoordhColor = texture(textTexture, TexCoord + vec2(gliTexCoordhFactor * 0.1 * iAmplitude, gliTexCoordhFactor * 0.1));
    
    vec4 mixResult = mix(mix(FragColor1, FragColor2, 0.5), gliTexCoordhColor, 0.5);
