...
void main(void) {
    ...
    vec4 colorLeft = texture(textTexture, TexCoord + vec2(-1.0 / iResolution.x, 0.0));
    vec4 colorRight = texture(textTexture, TexCoord + vec2(1.0 / iResolution.x, 0.0));
    vec4 colorUp = texture(textTexture, TexCoord + vec2(0.0, 1.0 / iResolution.y));
    vec4 colorDown = texture(textTexture, TexCoord + vec2(0.0, -1.0 / iResolution.y));
    vec4 avgColor = (texCol + colorLeft + colorRight + colorUp + colorDown) / 5.0;

    vec3 blended = mix(avgColor.rgb, rainbow_color, 0.5);
    vec3 finalCol = applyColorAdjustments(sin(blended * time_t));
