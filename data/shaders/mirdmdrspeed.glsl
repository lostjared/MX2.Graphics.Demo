...
void main() {
    ...
    float speedFactor = sin(time * 0.5 * iAmplitude) * 0.5 + 0.5;
    float angle1 = time * (0.2 + speedFactor * 0.8);
    float angle2 = time * (0.2 + (1.0 - speedFactor) * 0.8);
    ...
    FragColor = vec4(sin(FragColor.rgb * time), texColor.a);
}
