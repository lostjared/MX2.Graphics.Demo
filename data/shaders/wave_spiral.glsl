...
void main(void) {
    ...
    vec2 wave = vec2(sin(normCoord.y * waveFrequency + time) * waveStrength,
                     cos(normCoord.x * waveFrequency + time) * waveStrength);

    normCoord += wave;
    float dist = length(normCoord);
    float angle = atan(normCoord.y, normCoord.x);\
    float spiralAmount = tan(time) * 3.0;
    ...
    vec4 texCol = texture(textTexture, spiralCoord);
