...
void main(void) {
    ...
    float pingPongTime = pingPong(time, 9.0);
    ...
    float curtain_wave = sin(uv.y * 10.0 + time * 2.0) * 0.1;
    vec3 rainbow_color = rainbow((angle / (2.0 * 3.14159)) + curtain_wave, value_alpha_r);

    vec4 original = texture(textTexture, (rotatedUV + 1.0) / 2.0);
    vec3 blended = mix(original.rgb, rainbow_color, 0.5);
    vec3 finalCol = applyColorAdjustments(sin(blended * time_t));
    FragColor = vec4(finalCol, original.a);
}