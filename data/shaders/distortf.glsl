...
void main() {
    ...
    float fractalFactor = sin(length(offset) * 10.0 * iFrequency + time * 2.0) * 0.1 * iAmplitude;
    vec2 fractalUV = uv + fractalFactor * normalize(offset);
    float grid = abs(sin(fractalUV.x * 50.0) * sin(fractalUV.y * 50.0));
    grid = step(0.7, grid);
    float angle = atan(offset.y, offset.x) + fractalFactor * sin(time);
    ...
    vec3 rainbow = vec3(0.5 + 0.5 * sin(time + texColor.r),
                        0.5 + 0.5 * sin(time + texColor.g + 2.0),
                        0.5 + 0.5 * sin(time + texColor.b + 4.0));
