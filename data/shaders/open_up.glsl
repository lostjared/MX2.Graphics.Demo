...
void main(void) {
    ...
    float centerSize = 0.5 + 0.4 * sin(time * 3.0);
    float bloomEffect = sin(time + dist * 6.0 * iFrequency) * 0.5 + 0.5;
    ...
    vec4 mixResult = mix(centerColor, outerColor, mixFactor);
    vec4 mixResult2 = mix(mixResult, texture(textTexture, TexCoord), 0.5);
    vec3 col = applyColorAdjustments(mixResult2.rgb);
    FragColor = vec4(col, mixResult2.a);
}
