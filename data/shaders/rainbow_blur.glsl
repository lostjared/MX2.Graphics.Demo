...
void main(void) {
    ...
    vec3 rainbow_color = rainbow(uv.x + sin(time * 0.5 + uv.y * 5.0) * 0.1);

    vec4 blurred = blur(textTexture, TexCoord, iResolution);
    vec3 blended = mix(blurred.rgb, rainbow_color, 0.5);
    vec3 finalCol = applyColorAdjustments(sin(blended * time_t));
    FragColor = vec4(finalCol, blurred.a);
}
