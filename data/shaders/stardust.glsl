...
void main(void) {
    ...
    float starIntensity = smoothstep(starSize, starSize * 0.5 * iAmplitude, distance(starPos, vec2(starNoise, fract(sin(time + starNoise) * 0.5 + 0.5))));
    
    starIntensity *= 0.5 + 0.5 * sin(time * 10.0 * iFrequency + starNoise * 100.0);
    
    vec4 starColor = vec4(vec3(starIntensity), starIntensity);
    
    vec4 mixResult = mix(texColor, starColor, starColor.a);
