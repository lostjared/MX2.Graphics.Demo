...
void main(void) {
    ...
    vec2 uv = applyZoomRotation(TexCoord, vec2(0.5)) * iResolution.xy / iResolution.y;
    float pattern = sin(uv.x * 10.0 * iFrequency) * cos(uv.y * 10.0);

    pattern = (pattern + 1.0) * 0.5 * iAmplitude;

    float reveal = pingPong(time * 0.5, 1.0);
