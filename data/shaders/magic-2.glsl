...
void main(void) {
    ...
    float x = uv.x + sin(uv.y * 10.0 * iFrequency + time * 5.0) * 0.05 * iAmplitude;
    float y = uv.y + cos(uv.x * 10.0 + time * 5.0) * 0.05;
    vec2 displacedUV = vec2(x, y);
    vec4 texColor = texture(textTexture, displacedUV);
    float haze = sin(uv.y * 20.0 + time * 10.0) * 0.5 + 0.5;
    vec4 hazeColor = vec4(0.8, 0.3, 1.0, 1.0) * haze;
    FragColor = sin((texColor + hazeColor)*time_t);
}
