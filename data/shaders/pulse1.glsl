...
void main() {
    ...
    float ripple = sin(length(offset) * 20.0 * iFrequency - time * 5.0) * 0.05 * iAmplitude;
    float angle = atan(offset.y, offset.x) + ripple * sin(time);
    float radius = length(offset);
    vec2 swirlUV = center + radius * vec2(cos(angle), sin(angle));
    float pulse = 0.2 * sin(time * 3.0);
    swirlUV += pulse * normalize(offset);
    vec4 texColor = texture(textTexture, swirlUV);
    vec3 shiftedColor = texColor.rgb * (0.8 + 0.2 * sin(time * 2.0));
