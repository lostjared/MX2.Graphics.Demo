...
void main() {
    ...
    float zoom = 1.0 + 0.5 * sin(time);
    centeredUV *= zoom;

    float angle = time * 1.5 * zoom;
    ...
    float prismEffect = sin(prismAngle * 3.0) * 0.1 * iAmplitude;
    ...
    vec4 mixResult = mix(texColorOriginal, texColorPrism, 1.0 - blendFactor);
