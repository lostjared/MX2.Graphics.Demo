...
void main() {
    ...
    float spiral = angle + radius * 5.0 - time;
    ...
    vec4 gradientColor = vec4(0.5 + 0.5 * cos(time + radius * 10.0 * iFrequency), 0.5 + 0.5 * sin(time + radius * 10.0), 0.5 + 0.5 * cos(time - radius * 10.0), 1.0);
    vec4 textureColor = texture(textTexture, reflectUV);
    
    vec4 mixResult = mix(gradientColor, textureColor, 0.5);
