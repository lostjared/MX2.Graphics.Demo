...
void main(void) {
    ...
    float angle = atan(dir.y, dir.x) + time * 2.0;
    float radius = dist + sin(time * 2.0 + dist * 10.0 * iFrequency) * 0.05 * iAmplitude;
    vec2 spiral_TexCoord = center + vec2(cos(angle), sin(angle)) * radius;
    ...
    vec4 ctx = texture(textTexture, spiral_TexCoord);
    vec4 mixResult = mix(ctx, vec4(FragColor_gradient, 1.0), 0.5);
