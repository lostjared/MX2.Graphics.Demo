...
void main(void) {
    ...
    float wave = sin(distanceFromCenter * 12.0 - time * 4.0);
    vec2 TexCoordAdjusted = TexCoord + (normCoord * 0.301 * wave);
    vec4 textureColor = texture(textTexture, TexCoordAdjusted);

    vec4 mixResult = mix(textureColor, finalColor, 0.5);
