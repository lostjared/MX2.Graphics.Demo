...
void main() {
    ...
    vec4 col = blur(textTexture, TexCoord, iResolution);
    float value = pingPong(time, 10.0) + 2.0;
    float time_t =  pingPong(time, 7.0) + 2.0;
    FragColor = sin(FragColorRotate(col * value) * time_t);
}
