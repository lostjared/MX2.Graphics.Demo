...
void main(){
    ...
    vec3 grad = movingGradient(tc, m, time_f, aspect) * 0.5; // 1/2 brightness
    ...
    outCol *= 1.0 + _ab * 0.6;
    outCol = mix(outCol, outCol * vec3(1.0 + _abass * 0.3, 1.0 - _abass * 0.15, 1.0 + clamp(amp_high, 0.0, 1.0) * 0.25), _ab);
