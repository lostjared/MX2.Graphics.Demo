...
void main(void) {
    ...
    vec2 uv = applyZoomRotation(TexCoord, vec2(0.5)) * iResolution / vec2(iResolution.y);
    ...
    t = abs(t - 0.5);
    float time_t = mod(time, 10.0);
    vec4 texCol = texture(textTexture, TexCoord * r * time_t * t);
    vec3 finalCol = applyColorAdjustments(texCol.rgb);
    FragColor = vec4(finalCol, texCol.a); //vec2(TexCoord[0] * time_t * r, TexCoord[1] * time_t * t));
