...
void main(void) {
    ...
    float timeAdjustedFrequency = 10.0 + sin(time) * 5.0 * iFrequency;
    float timeAdjustedAmplitude = 0.01 + (sin(time * 0.5) * 0.5 + 0.5) * 0.05;
    ...
    float noise = rand(displacedCoord * time) * 0.1 * iAmplitude;
    float line = step(0.995, rand(vec2(displacedCoord.y * 100.0, time * 0.1)));
