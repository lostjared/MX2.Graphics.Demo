...
void main(void) {
    ...
    vec3 shifted = sin(blended * t * 2.0);
    vec4 result = vec4(shifted, texCol.a);
    ...
    result = (result * 0.4) + (FragColor2 * 0.4) + (FragColor3 * 0.4) + (FragColor4 * 0.4);

    vec3 finalCol = applyColorAdjustments(result.rgb);
    FragColor = vec4(finalCol, result.a);
}
