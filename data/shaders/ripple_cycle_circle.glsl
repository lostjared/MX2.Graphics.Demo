...
void main(void) {
    ...
    float angle = atan(centeredPos.y, centeredPos.x) + radius * 5.0 - time * 1.5;
    vec2 spiralOffset = radius * vec2(cos(angle), sin(angle));

    float wave = sin(mirroredPos.y * 10.0 + time * 2.0) * 0.02;
    vec2 waveOffset = vec2(sin(wave * time_t), 0);

    vec2 TexCoordAdjusted = TexCoord + vec2(phase * 0.302, phase * 0.302) + spiralOffset * 0.1 * iAmplitude + waveOffset;
