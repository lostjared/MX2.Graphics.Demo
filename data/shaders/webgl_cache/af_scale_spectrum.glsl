...
vec4 colorShift(vec4 col, float time) {
    return vec4(
        0.5 + 0.5 * cos(col.r * 3.14159265 * (time + 1.0)), // Adjust the phase shift based on time
        0.5 + 0.5 * cos(col.g * 3.14159265 * (time + 1.0)), // Adjust the phase shift here too
        0.5 + 0.5 * cos(col.b * 3.14159265 * (time + 1.0)), // And here
        col.a
    );
}

void mxCacheShaderMain() {
    float time_t = pingPong(time_f, 5.0) + 1.0;
    vec4 pix = blur(samp, tc, iResolution);
    pix = pix * time_t;
    pix = colorShift(pix, time_t); // Pass the current time to the color shift function
