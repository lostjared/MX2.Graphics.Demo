...
void main(void) {
    ...
    float boil = sin(uv.y * 20.0 * iFrequency + time * 6.0) * 0.2 * time;
    boil += cos(uv.x * 30.0 + time * 4.0) * 0.2 * time;
    boil += sin((uv.x + uv.y) * 25.0 + time * 5.0) * 0.1 * iAmplitude * time;
    uv.y += boil;
    uv.x += boil;
    ...
    vec4 mixResult = mix(baseTexture, distorted, 0.5);
