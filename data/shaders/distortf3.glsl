...
void main() {
    ...
    float expansion = cos(time * 3.0 - dist * 15.0) * 0.2;
    float spiral = sin(atan(offset.y, offset.x) * 3.0 + time * 2.0) * 0.1 * iAmplitude;
    ...
    texColor.rgb *= 1.0 + 0.3 * sin(dist * 20.0 - time * 5.0);

    vec3 finalCol = applyColorAdjustments(texColor.rgb);
