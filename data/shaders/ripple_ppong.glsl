...
void main(void) {
    ...
    vec2 uv = applyZoomRotation(TexCoord, vec2(0.5)) * iResolution;
    float distanceFromCenter = length(uv - center);

    float rippleSpeed = 2.0;
    float rippleFrequency = 20.0;

    time = time * rippleSpeed;
    ...
    vec2 rippleUV = TexCoord + normalize(uv - center) * wave * 0.01 * iAmplitude + warp;

    vec4 texCol = texture(textTexture, rippleUV);
