...
float diagonalDistance = (normCoord.x + normCoord.y - 1.0) * sqrt(2.0);
    ...
    float diagonalWave = sin((diagonalDistance + time) * 5.0 * iFrequency); // Wave frequency and speed
    float antiDiagonalWave = cos((antiDiagonalDistance + time) * 5.0);
    float combinedWave = (diagonalWave + antiDiagonalWave) * 0.5 * iAmplitude;

    vec2 waveAdjusted = vec2(TexCoord.x + combinedWave * 0.202, TexCoord.y + combinedWave * 0.202);
    
    vec4 texCol = texture(textTexture, waveAdjusted);
