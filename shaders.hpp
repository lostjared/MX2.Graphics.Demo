...
void main(void){
    ...
    vec2 uv=kaleido/ar+m;
    uv=fract(uv);
    color=texture(textTexture,sin(uv * (PI * pingPong(fract(time_f), 1.0))));
...
void main(void){
    ...
    float ca=0.0015+0.004*a;
    vec4 C=texture(textTexture,uv);
    C.r=texture(textTexture,fR+vec2( ca,0.0)).r;
    C.g=texture(textTexture,fG               ).g;
    C.b=texture(textTexture,fB+vec2(-ca,0.0)).b;
...
void main(void) {
    ...
    c += texture(textTexture, uvx + vec2(px.x, 0)).rgb;
    c += texture(textTexture, uvx + vec2(-px.x, 0)).rgb;
    c += texture(textTexture, uvx + vec2(0, px.y)).rgb;
    c += texture(textTexture, uvx + vec2(0, -px.y)).rgb;
    c *= 0.2;
